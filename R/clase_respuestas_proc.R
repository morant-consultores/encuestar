  ...
  R6::R6Class(
    ...
    public = list(
      ...
      q_patron = function(bd, dicc, patron){
        ...
        vec_llavs <- dicc$llaves[dicc$llaves %in% names(bd)]

        if(!is.na(patron)){
          aux <- bd %>%
            mutate(across(c(all_of(vec_llavs),where(is.character)),
      ...
      coordenadas_faltantes = function(){
        self$sin_coordenadas <- self$base %>% filter(is.na(as.numeric(Longitude)) | is.na(as.numeric(Latitude))) |>
          mutate(eliminada_proceso = 1)%>%
          mutate(razon = "Falta de coordenadas")
        self$base <- self$base %>% filter(!is.na(as.numeric(Longitude)) | !is.na(as.numeric(Latitude))) |>
          mutate(eliminada_proceso = 0)
      },
